...
void MessageGenerator::GenerateStaticVariablesHeader(io::Printer* printer) {
	...
	for (int i = 0; i < descriptor_->nested_type_count(); i++) {
		MessageGenerator(descriptor_->nested_type(i)).GenerateStaticVariablesHeader(
				printer);
	}
}
...
void MessageGenerator::GenerateStaticVariablesInitialization(
	...
	for (int i = 0; i < descriptor_->extension_count(); i++) {
		ExtensionGenerator(ClassName(descriptor_), descriptor_->extension(i)).GenerateInitializationSource(
				printer);
	}
	for (int i = 0; i < descriptor_->nested_type_count(); i++) {
		MessageGenerator(descriptor_->nested_type(i)).GenerateStaticVariablesInitialization(
				printer);
	}
}
...
void MessageGenerator::GenerateStaticVariablesSource(io::Printer* printer) {
	...
	for (int i = 0; i < descriptor_->extension_count(); i++) {
		ExtensionGenerator(ClassName(descriptor_), descriptor_->extension(i)).GenerateFieldsSource(
				printer);
	}

	for (int i = 0; i < descriptor_->nested_type_count(); i++) {
		MessageGenerator(descriptor_->nested_type(i)).GenerateStaticVariablesSource(
				printer);
	}
}
...
void MessageGenerator::DetermineDependencies(set<string>* dependencies) {
	...
	for (int i = 0; i < descriptor_->nested_type_count(); i++) {
		MessageGenerator(descriptor_->nested_type(i)).DetermineDependencies(
				dependencies);
	}
}
...
void MessageGenerator::GenerateEnumHeader(io::Printer* printer) {
	...
	for (int i = 0; i < descriptor_->nested_type_count(); i++) {
		MessageGenerator(descriptor_->nested_type(i)).GenerateEnumHeader(
				printer);
	}
}
...
void MessageGenerator::GenerateExtensionRegistrationSource(
		io::Printer* printer) {
	for (int i = 0; i < descriptor_->extension_count(); i++) {
		ExtensionGenerator(ClassName(descriptor_), descriptor_->extension(i)).GenerateRegistrationSource(
				printer);
	}

	for (int i = 0; i < descriptor_->nested_type_count(); i++) {
		MessageGenerator(descriptor_->nested_type(i)).GenerateExtensionRegistrationSource(
				printer);
	}
}
...
void MessageGenerator::GenerateMessageHeader(io::Printer* printer) {
	scoped_array<const FieldDescriptor*> sorted_fields(
			SortFieldsByType(descriptor_));

	if (descriptor_->extension_range_count() > 0) {
		printer->Print("@interface $classname$ : PBExtendableMessage {\n"
				"@private\n", "classname", ClassName(descriptor_));
	} else {
		printer->Print("@interface $classname$ : PBGeneratedMessage {\n"
				"@private\n", "classname", ClassName(descriptor_));
	}
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateHasPropertyHeader(
				printer);
	}
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GeneratePropertyHeader(
				printer);
	}
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateMembersHeader(
				printer);
	}
	...
	for (int i = 0; i < descriptor_->extension_count(); i++) {
		ExtensionGenerator(ClassName(descriptor_), descriptor_->extension(i)).GenerateMembersHeader(
				printer);
	}
	...
	for (int i = 0; i < descriptor_->nested_type_count(); i++) {
		MessageGenerator(descriptor_->nested_type(i)).GenerateMessageHeader(
				printer);
	}
...
void MessageGenerator::GenerateSource(io::Printer* printer) {
	printer->Print("@interface $classname$ ()\n", "classname",
			ClassName(descriptor_));
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateExtensionSource(
				printer);
	}
	printer->Print("@end\n\n");
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateSynthesizeSource(
				printer);
	}
	...
	printer->Indent();
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateDeallocSource(
				printer);
	}
	printer->Outdent();
	printer->Print("  [super dealloc];\n"
			"}\n");

	printer->Print("- (id) init {\n"
			"  if ((self = [super init])) {\n");
	printer->Indent();
	printer->Indent();
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateInitializationSource(
				printer);
	}
	printer->Outdent();
	printer->Outdent();
	...
	for (int i = 0; i < descriptor_->extension_count(); i++) {
		ExtensionGenerator(ClassName(descriptor_), descriptor_->extension(i)).GenerateMembersSource(
				printer);
	}

	printer->Print("static $classname$* default$classname$Instance = nil;\n"
			"+ (void) initialize {\n"
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateMembersSource(
				printer);
	}
...
void MessageGenerator::GenerateMessageSerializationMethodsHeader(
		io::Printer* printer) {
	scoped_array<const FieldDescriptor*> sorted_fields(
...
void MessageGenerator::GenerateSerializeOneFieldHeader(io::Printer* printer,
...
void MessageGenerator::GenerateBuilderHeader(io::Printer* printer) {
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		printer->Print("\n");
		field_generators_.get(descriptor_->field(i)).GenerateBuilderMembersHeader(
				printer);
	}
...
void MessageGenerator::GenerateCommonBuilderMethodsHeader(
		io::Printer* printer) {
	printer->Print("\n"
			"- ($classname$*) defaultInstance;\n"
			"\n"
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateBuildingCodeHeader(
				printer);
	}
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateMergingCodeHeader(
				printer);
	}
...
void MessageGenerator::GenerateBuilderParsingMethodsHeader(
		io::Printer* printer) {
	scoped_array<const FieldDescriptor*> sorted_fields(
			SortFieldsByNumber(descriptor_));
...
void MessageGenerator::GenerateIsInitializedHeader(io::Printer* printer) {
	printer->Print("- (BOOL) isInitialized;\n");
}

void MessageGenerator::GenerateMessageSerializationMethodsSource(
		io::Printer* printer) {
	scoped_array<const FieldDescriptor*> sorted_fields(
			SortFieldsByNumber(descriptor_));
	...
	for (int i = 0, j = 0;
		...
		if (i == descriptor_->field_count()) {
			GenerateSerializeOneExtensionRangeSource(printer,
					sorted_extensions[j++]);
	...
	if (descriptor_->options().message_set_wire_format()) {
		printer->Print("[self.unknownFields writeAsMessageSetTo:output];\n");
	} else {
		printer->Print(
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(sorted_fields[i]).GenerateSerializedSizeCodeSource(
				printer);
	}

	if (descriptor_->extension_range_count() > 0) {
		printer->Print("size += [self extensionsSerializedSize];\n");
	}
	...
	if (descriptor_->options().message_set_wire_format()) {
		...
		printer->Print("size += self.unknownFields.serializedSize;\n");
	}
...
void MessageGenerator::GenerateSerializeOneFieldSource(io::Printer* printer,
		const FieldDescriptor* field) {
...
void MessageGenerator::GenerateSerializeOneExtensionRangeSource(
		io::Printer* printer, const Descriptor::ExtensionRange* range) {
	printer->Print("[self writeExtensionsToCodedOutputStream:output\n"
			"                                    from:$from$\n"
			"                                      to:$to$];\n", "from",
			SimpleItoa(range->start), "to", SimpleItoa(range->end));
}
...
void MessageGenerator::GenerateBuilderSource(io::Printer* printer) {
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateBuilderMembersSource(
				printer);
	}
...
void MessageGenerator::GenerateCommonBuilderMethodsSource(
		io::Printer* printer) {
	if (descriptor_->extension_range_count() > 0) {
		printer->Print("- (PBExtendableMessage*) internalGetResult {\n"
				"  return protobufBuilderResult;\n"
				"}\n");
	} else {
		printer->Print("- (PBGeneratedMessage*) internalGetResult {\n"
				"  return protobufBuilderResult;\n"
				"}\n");
	}
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateBuildingCodeSource(
				printer);
	}
	...
	printer->Print("- ($classname$_Builder*) mergeFrom:($classname$*) other {\n"
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		field_generators_.get(descriptor_->field(i)).GenerateMergingCodeSource(
				printer);
	}
	...
	if (descriptor_->extension_range_count() > 0) {
		printer->Print("  [self mergeExtensionFields:other];\n");
	}

	printer->Print("  [self mergeUnknownFields:other.unknownFields];\n"
			"  return self;\n"
			"}\n");
}

void MessageGenerator::GenerateBuilderParsingMethodsSource(
		io::Printer* printer) {
	scoped_array<const FieldDescriptor*> sorted_fields(
			SortFieldsByNumber(descriptor_));
	...
	printer->Print("int32_t tag = [input readTag];\n"
			"switch (tag) {\n");
	printer->Indent();
...
void MessageGenerator::GenerateIsInitializedSource(io::Printer* printer) {
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		...
		if (field->is_required()) {
			printer->Print("if (!self.has$capitalized_name$) {\n"
					"  return NO;\n"
					"}\n", "capitalized_name",
					UnderscoresToCapitalizedCamelCase(field));
		}
	}
	...
	for (int i = 0; i < descriptor_->field_count(); i++) {
		...
		if (field->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE
			...
			case FieldDescriptor::LABEL_REQUIRED:
				printer->Print(vars, "if (!self.$name$.isInitialized) {\n"
						"  return NO;\n"
						"}\n");
				break;
			case FieldDescriptor::LABEL_OPTIONAL:
				printer->Print(vars, "if (self.has$capitalized_name$) {\n"
			...
			case FieldDescriptor::LABEL_REPEATED:
				printer->Print(vars, "for ($type$* element in self.$name$) {\n"
	...
	if (descriptor_->extension_range_count() > 0) {
		printer->Print("if (!self.extensionsAreInitialized) {\n"
				"  return NO;\n"
				"}\n");
	}

	printer->Outdent();
	printer->Print("  return YES;\n"
			"}\n");
}
} // namespace objectivec
} // namespace compiler
