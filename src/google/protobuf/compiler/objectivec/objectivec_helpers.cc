...
string UnderscoresToCapitalizedCamelCase(const string& input) {
	...
	for (int i = 0; i < input.size(); i++) {
		...
		if (c >= '0' && c <= '9') {
			...
			current += c;
			last_char_was_number = last_char_was_lower = last_char_was_upper =
					false;
...
string FilePath(const FileDescriptor* file) {
	...
	if (file->options().HasExtension(objectivec_file_options)) {
		ObjectiveCFileOptions options = file->options().GetExtension(
...
const char* const kKeywordList[] = { "TYPE_BOOL" };
		...
		if (field->type() == FieldDescriptor::TYPE_BYTES) {
			if (field->has_default_value()) {
				return "[NSData dataWithBytes:\""
						+ CEscape(field->default_value_string()) + "\" length:"
						+ SimpleItoa(field->default_value_string().length())
						+ "]";
			} else {
				return "[NSData data]";
			}
